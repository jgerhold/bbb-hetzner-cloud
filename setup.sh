...
curl -o /usr/local/bin/bbb-install.sh https://raw.githubusercontent.com/jgerhold/bbb-hetzner-cloud/main/bbb-install.sh
chmod +x /usr/local/bin/bbb-install.sh
/usr/local/bin/bbb-install.sh -c ${TURNDOMAINSECRET} -e ${EMAIL}
...
/usr/local/bin/update-config.sh --hcloud-token ${TOKEN} --floating-ips ${FLOATING_IPS}

#  -c <hostname>:<secret> Configure with coturn server at <hostname> using <secret>
curl -o /usr/local/bin/bbb-install.sh https://raw.githubusercontent.com/jgerhold/bbb-hetzner-cloud/main/bbb-install.sh
chmod +x /usr/local/bin/bbb-install.sh
/usr/local/bin/bbb-install.sh -w -g -v xenial-22 -s ${DOMAIN} -e ${EMAIL} -c ${TURNDOMAINSECRET}
